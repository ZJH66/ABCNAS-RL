...
from .LSTM import Encoder, Decoder, BiLSTM_Attention
from .BiGRU import BiGRU_Attention
from .GAE import GAEncoder, GADecoder
...
class Actor(nn.Module):
	...
	def __init__(self, config):
		...
		# self.encoder = GAEncoder(self.config)
		# self.decoder = GADecoder(self.config)
		# self.encoder = Encoder(self.config.max_length, self.config.input_dimension, self.config.hidden_dim)
		# decoder
		...
		elif self.config.decoder_type == 'BiLSTMDecoder':
			self.decoder = BiLSTM_Attention(config)
		elif self.config.decoder_type == 'BiGRUDecoder':
			self.decoder = BiGRU_Attention(config)
		else:
			raise NotImplementedError('Current decoder type is not implemented yet!')
		# self.decoder = Decoder(self.config.max_length, self.config.input_dimension, self.config.max_length, config)
		# self.decoder = BiLSTM_Attention(config)
	...
	def build_reward(self, reward_, i):

		self.reward = reward_.float()

		self.build_optim(i)

	def build_optim(self, i):
		...
		self.avg_baseline = self.alpha * self.avg_baseline + (1.0 - self.alpha) * reward_mean.cpu()
		if i == 1:
			self.pre_c = reward_mean
		else:
			self.pre_c = (self.pre_c * 0.9) + (0.1 * reward_mean)

		if self.config.device_type == 'gpu':
			self.avg_baseline = self.avg_baseline.cuda(self.config.device_ids)
		# Discounted reward
		self.reward_baseline = (self.reward - self.avg_baseline - self.pre_c).detach()  # [Batch size, 1]
		...
		self.loss1 = self.loss1.float()
		# Minimize step
		self.opt1.zero_grad()
		self.loss1.backward()
		nn.utils.clip_grad_norm_(
			list(self.encoder.parameters()) +
			list(self.decoder.parameters()), max_norm=1., norm_type=2)
