...
from datasets import load_dataset, IIDSimulation, DAG
...
class RL(BaseLearner):
    ...
    def learn(self, data, columns=None, dag=None, training=True, **kwargs):
        """
        ...
        if not training:
            config.nb_epoch = 1
            config.lambda_iter_num = 1
        causal_matrix = self._rl(X, config, training=training)
        self.causal_matrix = causal_matrix
        allmat.append(self.causal_matrix)

    # print(self.causal_matrix)
    # np.save("C:/Users/shertheus/Downloads/a.txt", self.causal_matrix)

    def _rl(self, X, config, training=True):
        # Reproducibility
        ...
        if config.lambda_flag_default:
            ...
            # print('Original sl: {}, su: {}, strue: {}'.format(sl, su, strue))
            # print('Transfomed sl: {}, su: {}, lambda2: {}, true: {}'.format(sl, su, lambda2,
        ...
        for i in (tqdm(range(1, config.nb_epoch + 1))):
            ...
            # Test tensor shape
            # if i == 1:
            #     print('Shape of actor.input: {}'.format(inputs.shape))

            # actor
            graphs_feed = actor(inputs.float(), training)
            if not training:
            ...
            if config.device_type == 'gpu':
                ...
                actor.build_reward(reward_=-torch.from_numpy(reward_feed)[:, 0].cuda(config.device_ids), i=i)
            else:
                reward_feed = callreward.cal_rewards(graphs_feed.detach().numpy(), lambda1, lambda2)  # np.array
                actor.build_reward(reward_=-torch.from_numpy(reward_feed)[:, 0], i=i)
            ...
            # logging
            # if i % 10000 == 0:
            #     print('[iter {}] reward_batch: {:.4}, max_reward: {:.4}, max_reward_batch: {:.4}'.
            ...
            if i % lambda_iter_num == 0:
                ...
                if hasattr(config, 'dag'):
                    ...
                    # print('before pruning: fdr {}, tpr {}, fpr {}, shd {}, nnz {}'.format(fdr, tpr, fpr, shd, nnz))
                    # print('after  pruning: fdr {}, tpr {}, fpr {}, shd {}, nnz {}'.format(fdr2, tpr2, fpr2, shd2, nnz2))

        # print('Training COMPLETED !')
        # if training:
        # 	torch.save(actor, act_path)
        return graph_batch_pruned.T
